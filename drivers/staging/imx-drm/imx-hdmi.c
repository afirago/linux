...
{
	writeb(val, hdmi->base + offset);
}

static inline u8 hdmi_readb(struct imx_hdmi *hdmi, int offset)
{
	return readb(hdmi->base + offset);
}
...
static void hdmi_set_clock_regenerator_cts(struct imx_hdmi *hdmi,
					   unsigned int cts)
{
	u8 val;
...
static void hdmi_set_clk_regenerator(struct imx_hdmi *hdmi, u32 pixel_clk_rate,
					u32 audio_rate, u32 hdmi_ratio)
{
	unsigned int clk_n, clk_cts;

	clk_n = hdmi_compute_n(audio_rate, pixel_clk_rate, hdmi_ratio);
	clk_cts = hdmi_compute_cts(audio_rate, pixel_clk_rate, hdmi_ratio);
	...
	pr_debug("%s: samplerate=%d  ratio=%d  pixelclk=%d  N=%d  cts=%d\n",
		__func__, audio_rate, hdmi_ratio, pixel_clk_rate,
		clk_n, clk_cts);

	hdmi_set_clock_regenerator_n(hdmi, clk_n);
	hdmi_set_clock_regenerator_cts(hdmi, clk_cts);
}

static void hdmi_clk_regenerator_update_pixel_clock(struct imx_hdmi *hdmi)
{
	u32 pixel_clk_rate;
	u32 audio_rate = 48000;	/* hard-coded for now */
	u32 ratio = 100;	/* always 100 in vendor-kernel */

	pixel_clk_rate = PICOS2KHZ(hdmi->fb_mode.pixclock) * 1000UL;
	hdmi_set_clk_regenerator(hdmi, pixel_clk_rate, audio_rate, ratio);
}
...
{
	enum hdmi_datamap color_format = 0;
	u8 val;

	if (hdmi->hdmi_data.enc_in_format == RGB) {
		if (hdmi->hdmi_data.enc_color_depth == 8)
			color_format = RGB444_8B;
		else if (hdmi->hdmi_data.enc_color_depth == 10)
			color_format = RGB444_10B;
		else if (hdmi->hdmi_data.enc_color_depth == 12)
			color_format = RGB444_12B;
		else if (hdmi->hdmi_data.enc_color_depth == 16)
			color_format = RGB444_16B;
		else
			return;
	} else if (hdmi->hdmi_data.enc_in_format == YCBCR444) {
		if (hdmi->hdmi_data.enc_color_depth == 8)
			color_format = YCbCr444_8B;
		else if (hdmi->hdmi_data.enc_color_depth == 10)
			color_format = YCbCr444_10B;
		else if (hdmi->hdmi_data.enc_color_depth == 12)
			color_format = YCbCr444_12B;
		else if (hdmi->hdmi_data.enc_color_depth == 16)
			color_format = YCbCr444_16B;
		else
			return;
	} else if (hdmi->hdmi_data.enc_in_format == YCBCR422_8BITS) {
		if (hdmi->hdmi_data.enc_color_depth == 8)
			color_format = YCbCr422_8B;
		else if (hdmi->hdmi_data.enc_color_depth == 10)
			color_format = YCbCr422_10B;
		else if (hdmi->hdmi_data.enc_color_depth == 12)
			color_format = YCbCr422_12B;
		else
			return;
	}
...
static void update_csc_coeffs(struct imx_hdmi *hdmi)
{
	...
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][0] & 0xFF),
		HDMI_CSC_COEF_A1_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][0] >> 8),
		HDMI_CSC_COEF_A1_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][1] & 0xFF),
		HDMI_CSC_COEF_A2_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][1] >> 8),
		HDMI_CSC_COEF_A2_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][2] & 0xFF),
		HDMI_CSC_COEF_A3_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][2] >> 8),
		HDMI_CSC_COEF_A3_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][3] & 0xFF),
		HDMI_CSC_COEF_A4_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[0][3] >> 8),
		HDMI_CSC_COEF_A4_MSB);

	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][0] & 0xFF),
		HDMI_CSC_COEF_B1_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][0] >> 8),
		HDMI_CSC_COEF_B1_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][1] & 0xFF),
		HDMI_CSC_COEF_B2_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][1] >> 8),
		HDMI_CSC_COEF_B2_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][2] & 0xFF),
		HDMI_CSC_COEF_B3_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][2] >> 8),
		HDMI_CSC_COEF_B3_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][3] & 0xFF),
		HDMI_CSC_COEF_B4_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[1][3] >> 8),
		HDMI_CSC_COEF_B4_MSB);

	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][0] & 0xFF),
		HDMI_CSC_COEF_C1_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][0] >> 8),
		HDMI_CSC_COEF_C1_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][1] & 0xFF),
		HDMI_CSC_COEF_C2_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][1] >> 8),
		HDMI_CSC_COEF_C2_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][2] & 0xFF),
		HDMI_CSC_COEF_C3_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][2] >> 8),
		HDMI_CSC_COEF_C3_MSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][3] & 0xFF),
		HDMI_CSC_COEF_C4_LSB);
	hdmi_writeb(hdmi, (unsigned char)(csc_coeff[2][3] >> 8),
					...
					unsigned char addr)
{
	...
	hdmi_writeb(hdmi, HDMI_PHY_I2CM_OPERATION_ADDR_READ,
		HDMI_PHY_I2CM_OPERATION_ADDR);
	hdmi_phy_wait_i2c_done(hdmi, 1000);
	msb = hdmi_readb(hdmi, HDMI_PHY_I2CM_DATAI_1_ADDR);
	lsb = hdmi_readb(hdmi, HDMI_PHY_I2CM_DATAI_0_ADDR);
	data = (msb << 8) | lsb;
	return data;
}
...
static void imx_hdmi_phy_init(struct imx_hdmi *hdmi)
{
	 ...
	 * This always fails because we force mDVI = true
	 */
...
static void hdmi_av_composer(struct imx_hdmi *hdmi)
{
	...
	dev_info(hdmi->dev, "x=%d, y=%d, refresh=%d\n", fb_mode->xres, fb_mode->yres, fb_mode->refresh);
...
static void imx_hdmi_setup(struct imx_hdmi *hdmi, struct drm_display_mode *mode)
{
	...
	/*
	 * TODO: ->vic is used to setup CEA modes if requested but is not
	 * handled at the moment. Could probably be renamed to something more
	 * meaningful as well :)
	 */
	...
	/*
	 * TODO: The following test isn't actually correct since we aren't
	 * testing for CEA modes anyway. Even on an HDMI monitor, we will set
	 * .mDVI=true
	 */
	if (hdmi->vic == 0) {
	...
	/* TODO: Again, we are override the CEA functionality here */
	...
	/* TODO: not for DVI mode
	 * This is being skipped over because mDVI is always true.
	 */
...
static int imx_hdmi_connector_get_modes(struct drm_connector *connector)
{
	...
	if (!hdmi->ddc) {
		dev_err(hdmi->dev, "Unable to get modes via EDID\n");
		return 0;
	}
...
static int imx_hdmi_platform_probe(struct platform_device *pdev)
{
	...
	if (ddc_node) {
		...
		dev_warn(&pdev->dev, "dcc property not found\n");
	}
	...
	hdmi->base = devm_ioremap(&pdev->dev, iores->start, resource_size(iores));
	if (!hdmi->base) {
	...
	hdmi->hdmi_isfr_clk = devm_clk_get(hdmi->dev, "isfr");
	...
	hdmi->hdmi_iahb_clk = devm_clk_get(hdmi->dev, "iahb");
	...
	hdmi_set_clk_regenerator(hdmi, 65000000, 48000, 100);
